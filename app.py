...
from utils.biometric_logic import compare_faces, save_base64_image
import io
...
# Cloudinary Configuration (Permanent Forensic Storage)
cloudinary.config(
...
for d in [DB_DIR, UPLOAD_DIR, TILES_DIR, HONEY_DIR, HONEY_POOL, PROFILES_DIR]:
...
def to_static_filename(path_value):
    """Normalize DB path values. Detects if the path is a remote Cloudinary URL or a local static path."""
    if not path_value:
        return None
    val = str(path_value)
...
def cleanup_expired_keys(conn=None):
    """Forensic Utility: Permanently delete keys older than 15 minutes to prevent data leakage."""
    close_at_end = False
    if conn is None:
        conn = get_conn()
        close_at_end = True
    ...
    try:
        cur = conn.cursor()
        # Calculate the 15-minute safety threshold
        threshold = (datetime.datetime.now(datetime.UTC) - datetime.timedelta(minutes=15)).isoformat()
        cur.execute("DELETE FROM keys WHERE created_at < ?", (threshold,))
        conn.commit()
    except Exception as e:
        print(f"Forensic Cleanup Error: {e}")
    finally:
        if close_at_end:
            conn.close()
...
def init_db():
    ...
    needed_cols = [
        ...
        "honey_name", "honey_crime_type", "face_photo_path"
    ]
...
def send_email_key(email, name, key):
    """Send access key via email using Brevo API with Strict Sender Matching"""
    import requests
    
    brevo_api_key = os.getenv('BREVO_API_KEY')
    # CRITICAL: This exact name must match your Brevo Verified Sender
    sender_name = "Forensic System"
    sender_email = "pdhanushp13@gmail.com"
    
    if not brevo_api_key:
        print("Brevo API Key not configured. Using fallback log.")
    ...
    try:
        ...
        if response.status_code in [200, 201, 202]:
            log_event("system", "email_sent", f"Verification key sent to {email} for {name} via Brevo (Exact Sender)")
            flash(f"Access key sent successfully to {email}!", "success")
            return True
        else:
            print(f"Brevo API Error: {response.text}")
            log_event("system", "email_failed", f"Brevo API Error: {response.text}")
            flash(f"Email delivery delayed by provider. Access Key: {key}", "warning")
            return False
            
    except Exception as e:
        print(f"Failed to send email via Brevo: {str(e)}")
        log_event("system", "email_failed", f"Brevo Exception: {str(e)}")
        flash(f"Email service error. Access Key: {key}", "warning")
        return False
...
def login_verifier():
    if request.method == "POST":
        ...
        if user and check_password_hash(user["password_hash"], password):
            ...
            if not login_photo_b64:
                flash("Biometric verification (Selfie) is required to login.", "error")
                return redirect(url_for("index", auth_error=1, role="verifier"))

            # Get the registered profile photo path
            profile_photo_rel = user["profile_photo"] # This is like 'profiles/uuid.jpg'
            if not profile_photo_rel:
                # Legacy user check or missing photo
                session["verifier"] = display_name
                session["role"] = "verifier"
                log_event(f"verifier:{display_name}", "login", "Authorized (Legacy - No Biometrics)")
                return redirect(url_for("verifier_view"))

            # Temporarily save login selfie to compare
            temp_login_path = PROFILES_DIR / f"temp_login_{uuid.uuid4().hex}.jpg"
            save_base64_image(login_photo_b64, str(temp_login_path))

            master_photo_path = STATIC_DIR / profile_photo_rel
            
            # Compare Faces
            is_match, score = compare_faces(str(master_photo_path), str(temp_login_path))
            
            # Clean up temp file
            if temp_login_path.exists():
                os.remove(temp_login_path)

            if is_match:
                session["verifier"] = display_name
                session["role"] = "verifier"
                log_event(f"verifier:{display_name}", "login", f"Biometric Match: {score}%")
                return redirect(url_for("verifier_view"))
            else:
                log_event(f"anonymous", "biometric_failed", f"Failed face match ({score}%) for: {display_name}")
                flash(f"Biometric Match Failed ({score}%). Access Denied.", "error")
                return redirect(url_for("index", auth_error=1, role="verifier"))
        else:
...
def register_verifier():
    if request.method == "POST":
        ...
        photo_b64 = request.form.get("profile_photo_base64")

        if not photo_b64:
            flash("Identity Enrollment (Profile Photo) is required.", "error")
            return redirect(url_for("index", auth_error=1, role="verifier"))
        ...
        try:
            ...
            save_base64_image(photo_b64, str(save_path))
            
            db_photo_path = f"profiles/{filename}"
...
def admin_users():
    ...
    conn = get_conn()
    cur = conn.cursor()
    cur.execute("SELECT id, role, username, email, mobile_dna, profile_photo, created_at FROM users ORDER BY id DESC")
...
def admin_record_original(record_id):
    ...
    if photo_rel:
        if photo_rel.startswith("http"):
            return redirect(photo_rel)
...
def admin_record_honey(record_id):
    ...
    if honey_rel:
        if honey_rel.startswith("http"):
            return redirect(honey_rel)
...
def admin_keys():
    ...
    # Forensic Self-Cleaning: Delete expired keys before showing them to Admin
...
def admin_keys_send():
    ...
    if request.method == "POST":
        ...
        try:
            ...
            # Clean up other expired keys while we are here
            cleanup_expired_keys(conn)
...
def admin_keys_generate():
    ...
    try:
        ...
        cur.execute(
            "INSERT INTO keys(record_id, secret_key, valid, created_at) VALUES (?,?,?,?)",
            (record_id, secret_key, 1, get_indian_time().isoformat()),
        )
        conn.commit()
        conn.close()
        ...
        try:
            ...
            try:
                upload_result = cloudinary.uploader.upload(
                    str(file_path), 
                    folder="forensic_vault",
                    public_id=f"doc_{timestamp}_{name.replace(' ', '_')}"
                )
                photo_db_path = upload_result['secure_url']
            except Exception as e:
                print(f"Cloudinary Primary Upload Error: {e}")
                photo_db_path = f"static/records/primary_vault/{filename}"
            ...
            # --- CLOUDINARY UPLOAD (DECOY) ---
            try:
                decoy_upload = cloudinary.uploader.upload(
                    str(honey_path_abs), 
                    folder="decoy_vault",
                    public_id=f"decoy_{timestamp}_{h_data['name'].replace(' ', '_')}"
                )
                honey_db_path = decoy_upload['secure_url']
            except Exception as e:
                print(f"Cloudinary Decoy Upload Error: {e}")
                honey_db_path = f"static/records/decoy_vault/{honey_filename}"
            
            # Save to database
            conn = get_conn()
            cur = conn.cursor()
            cur.execute(
                "INSERT INTO criminals(name, mobile, crime_type, photo_path, tile_dir, honey_path, honey_name, honey_crime_type, created_at, arrest_date, face_photo_path) VALUES (?,?,?,?,?,?,?,?,?,?,?)",
                (
                    name,
                    mobile,
                    crime_type,
                    ...
                    h_data["crime_type"],
                    get_indian_time().isoformat(),
                    request.form.get("arrest_date") or get_indian_time().strftime("%Y-%m-%d"),
                    photo_db_path
                ),
            )
        ...
        try:
            ...
            cur.execute("""
                INSERT INTO criminals(
                    ...
                    age_gender, address, ps1, fir1, ps2, fir2, ps3, 
                    arrest_date, case1, ps4_section, case2, id_marks, 
                    honey_name, honey_crime_type, created_at, face_photo_path
                ) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)""",
                (
                    form_data["name"],
                    mobile,
            ...
            # FIX: Store raw key so Admin can see it (Short format as requested)
            cur.execute(
                "INSERT INTO keys(record_id, secret_key, valid, created_at) VALUES (?,?,?,?)",
                (record_id, secret_key, 1, get_indian_time().isoformat()),
            )
...
def admin_edit_record(record_id):
    ...
    if request.method == "POST":
        ...
        try:
            timestamp = get_indian_time().strftime("%Y%m%d_%H%M%S")
            # Use face_photo_path if available to avoid recursive document problem
            source_photo = record["face_photo_path"] or record["photo_path"]
            mugshot_path = BASE_DIR / source_photo.replace("static/", "static/") if source_photo else None
            ...
            cur.execute("""
                ...
                (
                    ...
                    honey_data["name"], honey_data["crime_type"], 
                    str(mugshot_path.relative_to(BASE_DIR)).replace("\\", "/"),
                    record_id
                )
            )
            conn.commit()
...
def verifier_retrieve():
    ...
    try:
        ...
        for row in all_keys:
            # FIX: Compare plain keys for forensics (Reverted from hashing)
            if row["secret_key"] == key: 
                # Verify Time-Lock (15 Minute Expiry)
                created_dt = datetime.datetime.fromisoformat(row["created_at"])
                age = datetime.datetime.now(datetime.UTC) - created_dt
                
                if age.total_seconds() <= 900: # 15 minutes * 60 seconds
                    target_key_data = row
                    break
                else:
                    # Key is expired - Log this as a potential intrusion attempt
                    log_event(f"verifier:{session.get('verifier')}", "expired_key_denied", f"Expired key {key[:8]}... used.")
                    # We don't break, keep looking in case there's another valid key (unlikely) or just fall through to Decoy.
...
def download_honey(record_id):
    ...
    try:
        ...
        if record and record["honey_path"]:
            path = str(record["honey_path"])
            if path.startswith("http"):
                # Download from Cloudinary in RAM and serve
                import requests
                r = requests.get(path)
                return send_file(io.BytesIO(r.content), as_attachment=True, download_name=f"official_record_H{record_id}.png")
            
            # Local legacy fallback
            photo_path = BASE_DIR / path.replace("static/", "static/")
            if photo_path.exists():
...
def download_record(record_id):
    ...
    try:
        ...
        path = str(record["photo_path"])
        if path.startswith("http"):
            # Download from Cloudinary in RAM and serve 
            # Note: For real forensics we might want to verify signature, but here we serve the secure_url
            import requests
            r = requests.get(path)
            return send_file(io.BytesIO(r.content), as_attachment=True, download_name=f"official_forensic_doc_{record_id}.png")
            
        # Local legacy fallback with decryption
        photo_path = BASE_DIR / path.replace("static/", "static/")
        if photo_path.exists():
            # Phase 2: Decrypt encrypted file from disk into RAM (Memory)
            from utils.image_processor import decrypt_file_to_bytes
